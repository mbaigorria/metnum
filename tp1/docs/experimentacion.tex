...
Para la generación de instancias, utilizamos $m = 3$, $n = 3$, $r_i = 10$, $r_e = 100$, $ninst = 1$, con $m$ y $n$ aumentando de a 1. Aumentamos estos valores para aumentar la dimension de la matriz A dado que lo que relevante al tiempo de ejecución final del algoritmo es la dimension total, no de donde proviene la misma.
...
Para la temperatura se tomo un valor inicial de 250, por cada iteracion se multiplico a este valor inicial por el numero de iteraciones. Se generaron 20 instancias distintas con los parámetros indicados, actualizando la temperatura interna de la forma descripta anteriormente.
...
Este gráfico tenemos los multiplos de la temperatura inicial en el eje X, el mismo confirma nuestra intuición física y los resultados de los experimentos anteriores.
